<template>
                ...
                <div class="pl-3">
                    <div class="mb-3 pb-3 border-b last:mb-0 last:pb-0 last:border-b-0 border-gray-2"
                        ...
                        <ul v-if="item.item_extras.length > 0 || item.instruction !== ''"
                            ...
                            <li class="flex-col gap-1" v-if="item.attachment_url">
                                <h3 class="capitalize text-xs w-fit whitespace-nowrap">{{
                                ...
                                <div class="flex items-center gap-2">
                                    <img 
                                        ...
                                        alt="attachment" 
                                        class="rounded object-cover cursor-pointer w-full"
                                        @click="openAttachmentModal(item.attachment_url, item.item_name)"
                                    />
                                    <!-- <a 
    ...
    <!-- Attachment Modal -->
    <div id="attachment-modal" ref="attachmentModal" class="modal ff-modal">
        <div class="modal-dialog max-w-[600px]" v-if="selectedAttachment">
            <div class="modal-header">
                <h3 class="modal-title text-base font-medium">{{ selectedAttachment.name }}</h3>
                <button class="modal-close lab-close-circle-line font-fill-danger lab-font-size-24" @click.prevent="closeAttachmentModal"></button>
            </div>
            <div class="modal-body">
                ...
                <div class="flex justify-center mt-4 gap-3">
                    ...
                    >
                        <i class="lab lab-external-link-line lab-font-size-16"></i>
                        <span class="text-sm">{{ $t('button open in new tab') || 'Open in New Tab' }}</span>
                    </a>
                    <button 
                        @click="downloadAttachment"
                        class="flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-600 text-white hover:bg-gray-700 transition-colors"
                    >
                        <i class="lab lab-download-line lab-font-size-16"></i>
                        <span class="text-sm">{{ $t('button download') || 'Download' }}</span>
                    </button>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        openAttachmentModal: function (url, name) {
            this.selectedAttachment = {
                url: url,
                name: name
            };
            const modalTarget = this.$refs.attachmentModal;
            modalTarget?.classList?.add("active");
            document.body.style.overflowY = "hidden";
        },
        closeAttachmentModal: function () {
            this.selectedAttachment = null;
            const modalDiv = this.$refs.attachmentModal;
        ...
        downloadAttachment: function () {
            if (this.selectedAttachment) {
                ...
                link.href = this.selectedAttachment.url;
                link.download = this.selectedAttachment.name + '_attachment';
