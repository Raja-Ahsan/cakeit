            ...
            <div class="modal-body">
                ...
                <div class="mb-6">
                    ...
                    <div class="relative">
                        ...
                        <label 
                            for="attachment-file"
                            class="flex items-center justify-center gap-2 w-full h-12 rounded-lg border border-[#D9DBE9] cursor-pointer hover:border-primary/30 transition-colors"
                        >
                            <i class="lab lab-upload font-fill-primary lab-font-size-16"></i>
                            <span class="text-xs text-heading">
                                {{ temp.attachment ? temp.attachment.name : ($t('Choose file') || 'Choose File') }}
                            </span>
                        </label>
                        <div v-if="temp.attachment" class="mt-2 flex items-center gap-2">
                            <div class="flex items-center gap-2 bg-[#F7F7FC] rounded-lg px-3 py-2">
                                <i class="lab lab-image font-fill-primary lab-font-size-16"></i>
                                <span class="text-xs text-heading">{{ temp.attachment.name }}</span>
                                <button 
                                    @click.prevent="removeAttachment" 
                                    type="button"
                                    class="text-danger hover:text-red-700 transition-colors"
                                >
                                    <i class="lab lab-close-circle-line lab-font-size-16"></i>
                                </button>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        handleFileUpload: function (event) {
            ...
            if (file) {
                ...
                if (!allowedTypes.includes(file.type)) {
                    alertService.error(this.$t('message.invalid_file_type') || 'Invalid file type. Please select a WebP, PNG, JPEG, or JPG file.');
                ...
                if (file.size > maxSize) {
                    alertService.error(this.$t('message.file_too_large') || 'File size too large. Maximum size is 5MB.');
                    this.$refs.fileInput.value = '';
                    return;
                }
                
                console.log('File selected:', file);
                this.temp.attachment = file;
                
                // Store file in sessionStorage for later use
                const reader = new FileReader();
                reader.onload = (e) => {
                    const storedFiles = JSON.parse(sessionStorage.getItem('pos_attachments') || '{}');
                    storedFiles[this.temp.item_id] = {
                        name: file.name,
                        type: file.type,
                        size: file.size,
                        file: e.target.result // base64 string
                    };
                    sessionStorage.setItem('pos_attachments', JSON.stringify(storedFiles));
                    console.log('File stored in sessionStorage:', storedFiles);
                };
        ...
        removeAttachment: function () {
            ...
            const storedFiles = JSON.parse(sessionStorage.getItem('pos_attachments') || '{}');
            delete storedFiles[this.temp.item_id];
            sessionStorage.setItem('pos_attachments', JSON.stringify(storedFiles));
            console.log('File removed from sessionStorage');
        },
        
        addToCart: function () {
            console.log('Adding to cart with attachment:', this.temp.attachment);
            this.itemArrays = [
                {
                    name: this.temp.name,
