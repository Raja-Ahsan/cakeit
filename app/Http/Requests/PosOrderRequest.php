...
class PosOrderRequest extends FormRequest
{
    ...
    {
        // Handle dining_table_id - convert 'null' string to null
        if ($this->has('dining_table_id')) {
            $diningTableId = $this->input('dining_table_id');
            if ($diningTableId === 'null' || $diningTableId === '' || $diningTableId === 0) {
                $this->merge(['dining_table_id' => null]);
            }
        }

        // Handle pos_payment_note - convert 'null' string to null
        if ($this->has('pos_payment_note')) {
            $posPaymentNote = $this->input('pos_payment_note');
            if ($posPaymentNote === 'null' || $posPaymentNote === '') {
                $this->merge(['pos_payment_note' => null]);
            }
        }

        // Handle pos_received_amount - convert 'null' string to null
        if ($this->has('pos_received_amount')) {
            $posReceivedAmount = $this->input('pos_received_amount');
            if ($posReceivedAmount === 'null' || $posReceivedAmount === '') {
                $this->merge(['pos_received_amount' => null]);
            }
        }
    }
